...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!notifications || typeof notifications !== 'object') {
      return NextResponse.json({ success: false, error: '알림 설정을 입력해주세요.' }, { status: 400 })
    }
    ...
    // Supabase Admin 클라이언트로 사용자 알림 설정 업데이트
    ...
    if (updateError) {
      ...
      return NextResponse.json({ success: false, error: '알림 설정 업데이트에 실패했습니다.' }, { status: 500 })
    }

    return NextResponse.json({
      success: true,
      data: {
        notifications: updateData.user.user_metadata?.notifications || notifications
      },
